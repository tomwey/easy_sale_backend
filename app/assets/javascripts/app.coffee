window.App =
  ...
  viewPartin: (from_user, token, partinID) ->
    
    successCallback = (pos) ->
      App._viewPartin(from_user, token, partinID, "#{pos.lng},#{pos.lat}");

    errorCallback   = (error) ->
      App._viewPartin(from_user, token, partinID, null)

    App.getLocation(successCallback, errorCallback)
  
  _viewPartin: (from_user, token, partinID, loc) ->
    i = SBUtils.getRandomString(8)
    ak = SBUtils.getAccessKey(i)
    $.ajax
      url: "/api/v1/partins/#{partinID}/view"
      type: "POST"
      data: { token: token, from_type: 1, loc: loc, i: i, ak: ak, from_user: from_user }
      success: (re) ->
        console.log(re)
      error: (er) ->
        console.log(er)
  ...
  viewHB: (hbid) ->
    successCallback = (pos) ->
      App._viewHB(hbid, "#{pos.lng},#{pos.lat}");

    errorCallback   = (error) ->
      App._viewHB(hbid, null)
  ...
  commitHBData: (eventId, payload, type) ->
    # console.log(eventId)
    # console.log(payload)
    token = App.getToken()
    url = ''
    ...
    $.ajax
      url:  url
      type: "POST"
      data: { token: token, payload: payload, from_user: $('#commitBtn').data('from-user'), i: i, ak: ak }
      success: (re) ->
  ...
  commitQuziHB: (eventId, i, ak, type) ->
    ...
    successCallback = (pos) ->
      ...
      App.commitHBData(eventId, payload, type)
  ...
  # 提交红包
  commitHB: (eventId, eventType) -> 
    if eventType == 'QuizRule' || eventType == 'Question'
      App.commitQuziHB(eventId, eventType)
    else if eventType == 'CheckinRule' || eventType == 'LocationCheckin'
      App.commitCheckinHB(eventId, eventType)
      # console.log('eee')
  ...
  grabHongbao: (el) -> 
    ...
    App.commitHB(eventId, eventType)
  
  commitPartinData: (token, eventId, payload, type) ->
    # token = App.getToken()
    i = SBUtils.getRandomString(8)
    ak = SBUtils.getAccessKey(i)
    
    url = "/api/v1/partins/#{eventId}/commit"
    $.ajax
      url:  url
      type: "POST"
      data: { token: token,payload: payload,from_user: $('#commitBtn').data('from-user'),i: i, ak: ak }
      success: (re) ->
        $('#commitBtn').button('reset')
        if re.code == 0
          window.location.href="/wx/share/partin_result?id=#{eventId}&token=#{token}&code=0&log_id=#{re.data.id}"
        else
          window.location.href="/wx/share/partin_result?id=#{eventId}&token=#{token}&code=1&message=#{re.message}"
      error: (error) ->
        $('#commitBtn').button('reset')
        alert(JSON.stringify(error))
        window.location.href="/wx/share/partin_result?id=#{eventId}&token=#{token}&code=#{error.code || 500}"
        
  commitQuziPartin: (token, eventId, type) ->
    if $("input[name='answerOption2']")[0]
      answer = $("input[name='answerOption2']")[0].val()
      if !answer
        $('#commitBtn').button('reset')
        alert('答案不能为空')
        return
    ...
    successCallback = (pos) ->
      # console.log(pos)
      payload = JSON.stringify({ answer: answer, location: "#{pos.lng},#{pos.lat}" })
      App.commitPartinData(token, eventId, payload, type)
    ...
    App.getLocation(successCallback, errorCallback)
#
  commitCheckinPartin: (token, eventId, type) ->
    successCallback = (pos) ->
      # console.log(pos)
      payload = JSON.stringify({ location: "#{pos.lng},#{pos.lat}" })
      App.commitPartinData(token, eventId, payload, type)
...
#
  # 提交广告
  commitPartin: (token, eventId, eventType) ->
    if eventType == 'Question'
      App.commitQuziPartin(token, eventId, eventType)
    else if eventType == 'CheckinRule' || eventType == 'LocationCheckin'
      App.commitCheckinPartin(token, eventId, eventType)
#
  # 参与广告
  takePartin: (el, token) ->
    # token = window.localStorage.getItem('token')
    ...
    eventType = $(el).data('type')
    # $('.grab-btn a').attr('disabled');
    App.commitPartin(token, eventId, eventType)
  
  # 发送分享统计
  sendShareStat: (token, eventId) ->
    i = SBUtils.getRandomString(8)
    ak = SBUtils.getAccessKey(i)
    $.ajax
  ...
  # 发送分享统计
  sendPartinShareStat: (from_user, token, partinID) ->
    successCallback = (pos) ->
      App._sendPartinShareStat(from_user, token, "#{pos.lng},#{pos.lat}", partinID);

    errorCallback   = (error) ->
      App._sendPartinShareStat(from_user, token, null, partinID);

    App.getLocation(successCallback, errorCallback)
    
  _sendPartinShareStat: (from_user, token, loc, partinID) ->
    i = SBUtils.getRandomString(8)
    ak = SBUtils.getAccessKey(i)
    $.ajax
      url: "/api/v1/share/partin"
      type: "POST"
      data: { token: token, loc: loc, partin_id: partinID, i: i, ak: ak, from_user: from_user }
